...
MotorProfile motor_profiles[4] = {
    ...
    { 100, "AK80-64", 0.01f, 0.05f, 50.0f, 2.0f, 0.0f, 0.0f, 30.0f, DUMBBELL_L_SHOULDER },
    // 2. AK60-6 (ID: 110) - 어깨 내회전/외회전
    { 110, "AK60-6(1)", 0.002f, 0.01f, 10.0f, 0.5f, 0.0f, 0.0f, 10.0f, DUMBBELL_L_SHOULDER },
    // 3. AK70-10 (ID: 120) - 팔꿈치 굴곡/신전
    { 120, "AK70-10", 0.004f, 0.011f, 15.0f, 1.0f, 0.0f, 0.0f, 15.0f, DUMBBELL_L_ELBOW },
    // 4. AK60-6 (ID: 130) - 손목 굴곡/신전
    { 130, "AK60-6(2)", 0.002f, 0.01f, 10.0f, 0.5f, 0.0f, 0.0f, 10.0f, DUMBBELL_L_WRIST }
};
...
void loop() {
    ...
    // 1. UDP 수신 (비동기 처리) - 버퍼에 쌓인 패킷을 모두 읽어 최신 패킷만 적용
    // int packetSize = Udp.parsePacket();
    // bool got_new_packet = false;
    // JetsonToM7_UDP cmd;

    // // [추가] 마지막으로 UDP 패킷을 받은 시간을 기억하는 변수
    ...
    //         }
    //     }
    //     packetSize = Udp.parsePacket(); // 버퍼에 다음 패킷이 또 있는지 확인
    // }

    // // 최신 패킷을 성공적으로 수신했다면 단 1번만 명령 처리
    // if (got_new_packet) {
    //     jetson_ip = Udp.remoteIP();
    //     jetson_port = Udp.remotePort();
    //     jetson_connected = true;
    ...
    // // ------------------------------------------------------------------------
    // // [지능형 안전 로직] 통신 타임아웃 감시 (Timeout Watchdog)
    // // 모터가 켜져 있는데(motor_enabled), Jetson으로부터 500ms(0.5초) 이상 패킷이 안 오면!
    // if (motor_enabled && (millis() - last_udp_rx_time > 500)) {
    //     Serial.println("[SAFETY] Jetson Communication Timeout! Forcing Motors Disable.");
        
    //     for (int i = 0; i < 4; i++) {
    //         motor.setMotorID(motor_profiles[i].id);
    //         motor.setTargetTorque(0.0f); // 안전을 위해 토크 0
    //         delay(2);
    //         motor.disable();             // 모터 전원 차단
    //         delay(2);
            
    //         m_states[i].prev_mode = CMD_IDLE; // 모드 초기화
    //     }
    //     motor_enabled = false;
    //     jetson_connected = false; // 연결 끊김 상태로 전환
    // }
    // // ------------------------------------------------------------------------
    ...
    //     // 루프 외부 전역 변수로 선언 (이전 목표 토크 저장용 배열)
    ...
    //         // [핵심 변경] 1. 토크 전송과 상태 수신을 한 번의 패킷으로 통합 처리!
    ...
    //         // 안전 로직 검사
    //         if (expected_online[i] && !s.online) {
    //             comm_failure = true;
    //         }

    //         // [기존과 동일] 2. 읽어온 상태를 바탕으로 다음 틱에 들어갈 목표 토크 계산
    //         float cmd_tau = 0.0f;
    ...
    //         else if (s.g_fixMode) { cmd_tau = run_fix_pos(i); }

    //         cmd_tau = clamp_torque(i, cmd_tau);
            
    //         // 3. 계산된 토크를 다음 루프 송신을 위해 저장
    ...
    //         // [수정 1] 기존의 readMotorState(i); 삭제.
    //         // 대신 이전 틱의 토크(prev_cmd_tau)를 명령함과 동시에 현재 상태를 1-Shot으로 읽어옵니다.
    //         if (motor.sendTorqueAndReadState(prev_cmd_tau[i], new_state)) {
    //             s.current_position = new_state.position;
    //             s.current_velocity = new_state.velocity;
    //             s.real_torque = new_state.torque;
    ...
    // (loop 내부, t_start = micros() 선언 이후 모터 제어 for문 돌기 직전)
    ...
    if (motor_enabled) {
        ...
        for (int i = 0; i < 4; i++) {
            ...
            readMotorState(i);
            if (expected_online[i] && !m_states[i].online) comm_failure = true;

            float cmd_tau = 0.0f;
            MotorState& s = m_states[i];
            
            // --- [핵심 수정] 제어 분기점 ---
            // 자세 변환 중에는 무조건 모두 FIX_POS(run_fix_pos)만 실행하여 목표 자세로 부드럽게 이동
            if (!posture_ready) {
                cmd_tau = run_fix_pos(i);
            } 
            // 자세 변환이 끝나면 원래 지시받은 모드로 동작
            else {
                if (s.g_cpmMode) { cmd_tau = run_cpm(i); } 
                else if (s.g_promMode) { cmd_tau = run_transparent(i); } 
                else if (s.g_aromMode) { cmd_tau = run_arom(i); } 
            ...
            // // --- [핵심 안전장치] 토크 급변 방지 (Slew Rate Limiter) ---
            // // 모터가 갑자기 엄청난 힘을 내어 과전류로 뻗는 것을 방지 (Soft-Start)
            // // 전체 루프가 약 100Hz이므로, 0.05Nm/루프 = 1초에 최대 5Nm까지만 서서히 증가함
            // float max_delta_tau = 0.05f; 
            
            // if (cmd_tau > s.motor_output_torque + max_delta_tau) {
            //     cmd_tau = s.motor_output_torque + max_delta_tau;
            // } else if (cmd_tau < s.motor_output_torque - max_delta_tau) {
            //     cmd_tau = s.motor_output_torque - max_delta_tau;
            // }
            // // -------------------------------------------------------------
            ...
            // // [수정 2] 기존의 motor.setTargetTorque(cmd_tau); 삭제!!!
            // // 여기서 통신으로 보내지 않고, 다음 루프의 1-Shot 통신을 위해 배열에 담아둡니다.
            // prev_cmd_tau[i] = cmd_tau;
            s.motor_output_torque = cmd_tau;
