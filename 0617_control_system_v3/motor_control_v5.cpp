    ...
    void udpReceiveLoop() {
        ...
        while (is_running_) {
            ...
            if (n == sizeof(M7ToJetson_UDP)) {
                ...
                // // 지연시간 측정 계산
                // uint64_t tx_time = tx_timestamps_[rx_data.seq_echo % 1024];
                // uint64_t total_rtt_us = now_us - tx_time;
                
                // // 1초마다 터미널에 지연 시간 및 모터 상태 출력
                // RCLCPP_INFO_THROTTLE(this->get_logger(), *this->get_clock(), 1000, 
                //     "[%s] Latency | Total RTT: %ld us | FDCAN RTT: %u us | M7 Process: %u us\n"
                ...
                //     arm.c_str(), total_rtt_us, rx_data.fdcan_rtt_us, rx_data.m7_rx_to_tx_us,
                ...
                // // 마이크로초(us)를 헤르츠(Hz)로 변환 계산 (0으로 나누기 방지)
                // double total_hz = total_rtt_us > 0 ? 1000000.0 / total_rtt_us : 0.0;
                // double fdcan_hz = rx_data.fdcan_rtt_us > 0 ? 1000000.0 / rx_data.fdcan_rtt_us : 0.0;
                // double m7_process_hz = rx_data.m7_rx_to_tx_us > 0 ? 1000000.0 / rx_data.m7_rx_to_tx_us : 0.0;
                
                // // 1초마다 터미널에 지연 시간, 주파수(Hz) 및 모터 상태 출력
                // RCLCPP_INFO_THROTTLE(this->get_logger(), *this->get_clock(), 1000, 
                //     "[%s] Latency & Freq | Total RTT: %ld us (%.1f Hz)\n"
                //     "      FDCAN RTT: %u us (%.1f Hz) | M7 Process: %u us (%.1f Hz)\n"
                //     "      [0] Pos: %5.2f | Vel: %5.2f | TauCmd: %6.2f | TauMeas: %6.2f\n"
                //     "      [1] Pos: %5.2f | Vel: %5.2f | TauCmd: %6.2f | TauMeas: %6.2f\n"
                //     "      [2] Pos: %5.2f | Vel: %5.2f | TauCmd: %6.2f | TauMeas: %6.2f\n"
                //     "      [3] Pos: %5.2f | Vel: %5.2f | TauCmd: %6.2f | TauMeas: %6.2f", 
                //     arm.c_str(), 
                //     total_rtt_us, total_hz, 
                //     rx_data.fdcan_rtt_us, fdcan_hz, 
                //     rx_data.m7_rx_to_tx_us, m7_process_hz,
                //     rx_data.joints[0].pos, rx_data.joints[0].vel, rx_data.joints[0].tau_cmd, rx_data.joints[0].tau_meas,
                //     rx_data.joints[1].pos, rx_data.joints[1].vel, rx_data.joints[1].tau_cmd, rx_data.joints[1].tau_meas,
                //     rx_data.joints[2].pos, rx_data.joints[2].vel, rx_data.joints[2].tau_cmd, rx_data.joints[2].tau_meas,
                //     rx_data.joints[3].pos, rx_data.joints[3].vel, rx_data.joints[3].tau_cmd, rx_data.joints[3].tau_meas);
                ...
                //     double actual_rx_hz = rx_packet_count / ((now_us - last_print_time) / 1000000.0);
                    
                //     RCLCPP_INFO(this->get_logger(), 
                //         "[%s] Comm Rate: %.1f Hz (Target: 100Hz) | Latency(RTT): %ld us | M7 Process: %u us (%.1f Hz)\n"
                //         "      [0] Pos: %5.2f | Vel: %5.2f | TauCmd: %6.2f | TauMeas: %6.2f\n"
                //         "      [1] Pos: %5.2f | Vel: %5.2f | TauCmd: %6.2f | TauMeas: %6.2f\n"
                //         "      [2] Pos: %5.2f | Vel: %5.2f | TauCmd: %6.2f | TauMeas: %6.2f\n"
                //         "      [3] Pos: %5.2f | Vel: %5.2f | TauCmd: %6.2f | TauMeas: %6.2f", 
                //         arm.c_str(), 
                //         actual_rx_hz,  // 실제 1초간 받은 패킷 갯수
                //         total_rtt_us,  // 현재 패킷의 지연 시간
                //         rx_data.m7_rx_to_tx_us, m7_process_hz, // M7 처리 속도
                //         rx_data.joints[0].pos, rx_data.joints[0].vel, rx_data.joints[0].tau_cmd, rx_data.joints[0].tau_meas,
                //         rx_data.joints[1].pos, rx_data.joints[1].vel, rx_data.joints[1].tau_cmd, rx_data.joints[1].tau_meas,
                //         rx_data.joints[2].pos, rx_data.joints[2].vel, rx_data.joints[2].tau_cmd, rx_data.joints[2].tau_meas,
                //         rx_data.joints[3].pos, rx_data.joints[3].vel, rx_data.joints[3].tau_cmd, rx_data.joints[3].tau_meas);
